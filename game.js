...
const REMOTE_STATE_INTERVAL=110; // V5.58: t3.small 준비 전까지 Supabase Realtime 사용
...
const CANNON_Q_BEHAVIOR=Object.freeze({
  ...
  shredder:'shredder',seeker:'seeker',frost:'frost',magnet:'magnet',
  cyclone:'cyclone',juggernaut:'juggernaut',mirror:'mirror',lancer:'lancer',
  leviathan:'leviathan',valkyrie:'valkyrie',berserker:'berserker',oracle:'oracle'
});
const CANNON_R_BEHAVIOR=Object.freeze({
  cyclone:'cyclone',juggernaut:'juggernaut',mirror:'mirror',lancer:'lancer',
  leviathan:'leviathan',valkyrie:'valkyrie',berserker:'berserker',oracle:'oracle'
});
const CANNON_STAT_MODS=Object.freeze({
  blaster:{damage:1.15,reload:1.10},ranger:{damage:.92,bulletSpeed:1.10,reload:.95,move:1.08},
  ricochet:{damage:1.05,reload:.95},mortar:{damage:1.12,bulletSpeed:.86,reload:1.18},
  shredder:{damage:1.12,reload:1.06},seeker:{damage:1.08,bulletSpeed:1.12},
  frost:{damage:1.10,bulletSpeed:.95,reload:1.05},magnet:{damage:1.15,bulletSpeed:.90,reload:1.08},
...
function cannonRBehavior(id){return CANNON_R_BEHAVIOR[id]||id}
function cannonFamily(id){return CANNON_FAMILY[id]||CANNON_FAMILY[cannonBaseBehavior(id)]||'standard'}
...
const CANNON_SKILLS=Object.freeze({
  ...
  blaster:{name:'코어 캐논',cooldown:15,color:'#8fc7ff'},
  ranger:{name:'헌터 스텝',cooldown:13,color:'#7ee8ff'},
  ricochet:{name:'핀볼 코어',cooldown:17,color:'#70f0d8'},
  mortar:{name:'융단 박격',cooldown:18,color:'#ffc27d'},
  shredder:{name:'톱날 폭풍',cooldown:20,color:'#d3a8ff'},
...
const CANNON_SKILLS_2=Object.freeze({
  ...
  cyclone:{name:'VORTEX PRISON',cooldown:30,color:'#c9d6ff',kind:'vortex'},
  juggernaut:{name:'KINETIC REVENGE',cooldown:29,color:'#ffd39a',kind:'counterArmor'},
  mirror:{name:'MIRROR WORLD',cooldown:28,color:'#f1d7ff',kind:'mirrorWorld'},
  lancer:{name:'DIMENSION IMPALE',cooldown:27,color:'#d9e8ff',kind:'phaseImpale'},
...
const TANK_THEMES=Object.freeze({
  ...
  frost:{body:'#5d91b5',edge:'#32556f',glow:'#c0f3ff'},magnet:{body:'#6b4f8e',edge:'#37284e',glow:'#c49aff'},
...
const DIEP_EVOLUTION_INFO=Object.freeze({
  basic:{name:'탱크',tier:1,desc:'기본 탱크'},
  twin:{name:'트윈',tier:15,desc:'두 포신 · 빠른 연사',mods:{damage:.94,reload:.84}},
  sniperClass:{name:'스나이퍼',tier:15,desc:'긴 사거리 · 느린 연사',mods:{damage:1.12,bulletSpeed:1.16,reload:1.12}},
  machineGun:{name:'머신 건',tier:15,desc:'빠른 연사 · 넓은 탄 분산',mods:{damage:.84,reload:.74}},
  ...
  trapper:{name:'트래퍼',tier:30,desc:'방어형 함정 포신',mods:{damage:1.15,bulletSpeed:.86,reload:1.16}},
  destroyer:{name:'디스트로이어',tier:30,desc:'초대형 고화력 단발',mods:{damage:1.62,bulletSpeed:.78,reload:1.72,move:.96}},
  gunner:{name:'거너',tier:30,desc:'고속 집탄',mods:{damage:.78,bulletSpeed:1.10,reload:.66}},
  shotgun:{name:'샷건',tier:30,desc:'근거리 파편 포격',mods:{damage:1.08,bulletSpeed:.90,reload:1.22}},
  triAngle:{name:'트라이-앵글',tier:30,desc:'후방 반동 가속',mods:{damage:.94,reload:.90,move:1.16}},
  auto3:{name:'오토 3',tier:30,desc:'3방향 자동 포탑형',mods:{damage:.90,reload:.80}},
  smasher:{name:'스매셔',tier:30,desc:'충돌 기동 특화',mods:{damage:1.06,reload:1.10,move:1.18}},

  triplet:{name:'트리플렛',tier:45,desc:'3연장 집탄 최종형',mods:{damage:1.02,reload:.67}},
  pentaShot:{name:'펜타 샷',tier:45,desc:'전방 5방향 탄막',mods:{damage:.90,reload:.70,move:1.04}},
  spreadShot:{name:'스프레드 샷',tier:45,desc:'초광각 전방 탄막',mods:{damage:.86,reload:.68}},
  octoTank:{name:'옥토 탱크',tier:45,desc:'8방향 전방위 탄막',mods:{damage:.86,reload:.65}},
  auto5:{name:'오토 5',tier:45,desc:'5방향 자동 포탑형',mods:{damage:.92,reload:.72}},
  tripleTwin:{name:'트리플 트윈',tier:45,desc:'3축 쌍포신',mods:{damage:.86,reload:.64}},
  ranger:{name:'레인저',tier:45,desc:'최장거리 저격',mods:{damage:1.32,bulletSpeed:1.34,reload:1.30}},
  stalker:{name:'스토커',tier:45,desc:'은밀 저격 특화',mods:{damage:1.28,bulletSpeed:1.24,reload:1.24,move:1.03}},
  predator:{name:'프레데터',tier:45,desc:'강력한 장거리 포식자',mods:{damage:1.22,bulletSpeed:1.26,reload:1.08}},
  streamliner:{name:'스트림라이너',tier:45,desc:'초고속 단방향 연사',mods:{damage:.80,bulletSpeed:1.14,reload:.54}},
  triTrapper:{name:'트라이-트래퍼',tier:45,desc:'3방향 함정 방어',mods:{damage:1.16,bulletSpeed:.84,reload:.92}},
  gunnerTrapper:{name:'거너 트래퍼',tier:45,desc:'거너 + 트래퍼 혼합',mods:{damage:.94,bulletSpeed:1.02,reload:.70}},
  megaTrapper:{name:'메가 트래퍼',tier:45,desc:'거대 함정 단발',mods:{damage:1.48,bulletSpeed:.72,reload:1.46}},
  autoTrapper:{name:'오토 트래퍼',tier:45,desc:'자동포 + 트래퍼',mods:{damage:1.02,bulletSpeed:.90,reload:.82}},
  annihilator:{name:'어나이얼레이터',tier:45,desc:'최대 구경 파괴포',mods:{damage:1.95,bulletSpeed:.72,reload:1.92,move:.92}},
  skimmer:{name:'스키머',tier:45,desc:'회전 보조탄 파괴포',mods:{damage:1.54,bulletSpeed:.82,reload:1.48}},
  glider:{name:'글라이더',tier:45,desc:'기동형 파괴포',mods:{damage:1.48,bulletSpeed:.90,reload:1.42,move:1.08}},
  rocketeer:{name:'로켓티어',tier:45,desc:'장거리 추진탄 파괴포',mods:{damage:1.56,bulletSpeed:1.00,reload:1.55}},
  autoGunner:{name:'오토 거너',tier:45,desc:'자동포가 결합된 거너',mods:{damage:.82,bulletSpeed:1.08,reload:.58}},
  sprayer:{name:'스프레이어',tier:45,desc:'머신 건 직계 최종형',mods:{damage:.90,reload:.55}},
  palletShot:{name:'팰릿 샷',tier:45,desc:'촘촘한 산탄 파편',mods:{damage:1.00,bulletSpeed:.94,reload:.92}},
  dualBarrel:{name:'듀얼 배럴',tier:45,desc:'쌍포 샷건 최종형',mods:{damage:1.08,bulletSpeed:.92,reload:.88}},
  booster:{name:'부스터',tier:45,desc:'최고속 후방 추진',mods:{damage:.92,reload:.86,move:1.28}},
  fighter:{name:'파이터',tier:45,desc:'기동 + 측면 화력',mods:{damage:1.00,reload:.82,move:1.22}},
  landmine:{name:'랜드마인',tier:45,desc:'충돌/은신형 스매셔',mods:{damage:1.18,move:1.20}},
  autoSmasher:{name:'오토 스매셔',tier:45,desc:'스매셔 + 자동포',mods:{damage:1.12,reload:.84,move:1.16}},
  spike:{name:'스파이크',tier:45,desc:'최강 충돌형 스매셔',mods:{damage:1.24,move:1.23}},
  autoTank:{name:'오토 탱크',tier:45,desc:'기본 탱크의 자동포 최종형',mods:{damage:1.08,reload:.82}}
});
const DIEP_EVOLUTION_CHILDREN=Object.freeze({
  basic:['twin','sniperClass','machineGun','flankGuard','smasher','autoTank'],
  twin:['tripleShot','quadTank','twinFlank'],
  sniperClass:['assassin','hunter','trapper'],
  machineGun:['destroyer','gunner','shotgun','sprayer'],
  flankGuard:['triAngle','quadTank','twinFlank','auto3'],
  tripleShot:['triplet','pentaShot','spreadShot'],
  quadTank:['octoTank','auto5'],
  twinFlank:['tripleTwin'],
  assassin:['ranger','stalker'],
  hunter:['predator','streamliner'],
  trapper:['triTrapper','gunnerTrapper','megaTrapper','autoTrapper'],
  destroyer:['annihilator','skimmer','glider','rocketeer'],
  gunner:['autoGunner','gunnerTrapper','streamliner'],
  shotgun:['palletShot','dualBarrel'],
  triAngle:['booster','fighter'],
  auto3:['auto5','autoGunner'],
  smasher:['landmine','autoSmasher','spike']
});
const EVOLUTION_MILESTONES=[15,30,45];
function eligibleEvolutionChildren(type,level){
  return (DIEP_EVOLUTION_CHILDREN[type]||[]).filter(id=>(DIEP_EVOLUTION_INFO[id]?.tier||99)<=level);
}
...
function upsertRemotePlayer(payload){
  ...
  r.level=clamp(Math.floor(safeRemoteNumber(payload.level,r.level)),1,45);
  ...
  r.classType=String(payload.classType||r.classType||'basic');r.evolutionTier=Math.max(0,Math.floor(safeRemoteNumber(payload.evolutionTier,r.evolutionTier||0)));
...
function localNetworkState(){
  ...
  return {
    ...
    classType:player.classType||'basic',evolutionTier:player.evolutionTier||0,alive:!!player.alive,
...
function broadcastShot(b){
  ...
  sendOnline('shot',{
    ...
    targetId:String(b.targetId||''),targetShapeId:String(b.targetShapeId||''),
...
function gainXp(a){
  if(!player.alive)return;
  const gain=Math.max(0,Number(a)||0);
  player.score+=Math.round(gain*3);
  if(player.level>=45){player.level=45;player.xp=0;updateUI();return}
  player.xp+=gain;
  while(player.level<45&&player.xp>=player.xpNeed){
...
function gainPolygonXp(rawXp){
  const full=Math.max(0,Number(rawXp)||0);
  if(!player.alive)return;
  const scoreBefore=player.score;
  gainXp(full*.5);
  // gainXp couples score to XP; restore polygon score to its original full value.
  player.score=scoreBefore+Math.round(full*3);
  updateUI();
}
...
function playerParams(){
  ...
  const evoMods=DIEP_EVOLUTION_INFO[player.classType||'basic']?.mods||null;
  if(evoMods){
    damage*=Number(evoMods.damage)||1;
    bulletSpeed*=Number(evoMods.bulletSpeed)||1;
    reload*=Number(evoMods.reload)||1;
    move*=Number(evoMods.move)||1;
  }

  const cannon=player.cannonType||'standard';
  const baseCannon=cannonBaseBehavior(cannon);

  if(baseCannon==='rapid'){damage*=.48;bulletSpeed*=1.06;reload*=.36}
  else if(baseCannon==='spread'){damage*=.50;bulletSpeed*=.90;reload*=1.30}
  else if(baseCannon==='piercer'){damage*=1.15;bulletSpeed*=1.42;reload*=1.48}
  else if(baseCannon==='plasma'){damage*=1.35;bulletSpeed*=.72;reload*=1.62}
  else if(baseCannon==='rocket'){damage*=2.15;bulletSpeed*=.60;reload*=2.20}
  else if(baseCannon==='ring'){damage*=.86;bulletSpeed*=1.05;reload*=1.15}
  else if(baseCannon==='nova'){damage*=1.20;bulletSpeed*=.92;reload*=1.42}
  else if(baseCannon==='error'){damage*=2.55;bulletSpeed*=1.60;reload*=1.75}

  else if(baseCannon==='scout'){damage*=.86;bulletSpeed*=1.22;reload*=.78;move*=1.12}
  else if(baseCannon==='bastion'){damage*=1.38;bulletSpeed*=.86;reload*=1.42;move*=.90}

  else if(baseCannon==='dual'){damage*=.52;bulletSpeed*=1.08;reload*=.52}
  else if(baseCannon==='needle'){damage*=.62;bulletSpeed*=1.35;reload*=.56;move*=1.05}

  else if(baseCannon==='burst'){damage*=.56;bulletSpeed*=.96;reload*=1.05}
  else if(baseCannon==='crystal'){damage*=.50;bulletSpeed*=1.02;reload*=1.17}

  else if(baseCannon==='laser'){damage*=1.08;bulletSpeed*=1.62;reload*=1.36}
  else if(baseCannon==='drill'){damage*=1.55;bulletSpeed*=1.16;reload*=1.78;move*=.94}

  else if(baseCannon==='thunder'){damage*=1.20;bulletSpeed*=.78;reload*=1.45}
  else if(baseCannon==='inferno'){damage*=1.68;bulletSpeed*=.68;reload*=1.72}

  else if(baseCannon==='titan'){damage*=2.48;bulletSpeed*=.55;reload*=2.42;move*=.84}
  else if(baseCannon==='phantom'){damage*=1.34;bulletSpeed*=1.66;reload*=1.25;move*=1.16}

  else if(baseCannon==='chrono'){damage*=.94;bulletSpeed*=1.14;reload*=1.06;move*=1.06}
  else if(baseCannon==='void'){damage*=1.36;bulletSpeed*=.86;reload*=1.50}

  else if(baseCannon==='comet'){damage*=1.16;bulletSpeed*=1.26;reload*=1.16;move*=1.10}
  else if(baseCannon==='stellar'){damage*=1.36;bulletSpeed*=.91;reload*=1.52}

  else if(baseCannon==='glitch'){damage*=2.10;bulletSpeed*=1.52;reload*=1.38;move*=1.05}
  ...
  if(cannon!=='sniper')player.sniperScopeUntil=0;
  if(cannon==='deku'){
...
function receiveRemoteSkill(payload){
  ...
  if(skillType){
    ...
    if(cannon==='gojo'&&skillType==='gojoPurple'){
      const charge=Math.max(.35,safeRemoteNumber(payload.charge,.72)),flight=Math.max(.35,safeRemoteNumber(payload.flight,.78));
      skillZones.push({type:'gojoPurpleProjectile',x,y,radius,life:charge+flight,maxLife:charge+flight,damage:0,tick:0,angle:travelAngle,length:Math.min(1200,safeRemoteNumber(payload.length,980)),ownerId:String(payload.ownerId||''),networkRemote:true,pulses:0,interval:.4,data:{charge,flight}});
      return;
    }
    if(cannon==='gojo'&&skillType==='gojoAoEnd'){
      for(let i=skillZones.length-1;i>=0;i--){const z=skillZones[i];if(z.networkRemote&&z.ownerId===String(payload.ownerId||'')&&z.type==='gojoAo')skillZones.splice(i,1)}
      return;
    }
    ...
    if(zoneTypes.has(skillType)){
      if(skillType==='gojoAo'&&payload.held===true){for(let i=skillZones.length-1;i>=0;i--){const z=skillZones[i];if(z.networkRemote&&z.ownerId===String(payload.ownerId||'')&&z.type==='gojoAo')skillZones.splice(i,1)}}
...
function skillProjectile(cannon,angle,opts={}){
  ...
  if(opts.targetId)b.targetId=String(opts.targetId);
  if(opts.targetShapeId)b.targetShapeId=String(opts.targetShapeId);
...
function skillAreaDamage(x,y,radius,damage,color){
  ...
  for(let i=shapes.length-1;i>=0;i--){
    ...
    if(s.hp<=0){
      gainPolygonXp(s.xp);burst(s.x,s.y,colorForShape(s.type),11);shapes.splice(i,1);
    }
  }
...
function setSkillButtonVisible(button,visible){
  if(!button)return;
...
function setFourthSkillVisible(visible){
  setSkillButtonVisible(ui.skill4Btn,visible);
  if(ui.skill4Btn&&!visible)ui.skill4Btn.disabled=true;
}
function updateSkillHud(){
  ...
  if(ui.skillHud){
    ui.skillHud.dataset.cannon=cannon;
    ui.skillHud.dataset.skillCount=cannon==='deku'?'4':String(1+(def2?1:0)+((cannon==='error'||cannon==='deku')?1:0)+(cannon==='deku'?1:0));
  }
  setSkillButtonVisible(ui.skillBtn,true);
  if(cannon==='deku'){
    setSkillButtonVisible(ui.skill2Btn,true);
    setSkillButtonVisible(ui.skill3Btn,true);
    setSkillButtonVisible(ui.skill4Btn,true);
  }
  ...
  if(ui.skillCooldown){
    ...
    ui.skillCooldown.textContent=errorCharging
      ...
      : (phantomReturn?`귀환 ${phantomLeft.toFixed(1)}s`:
        (cannon==='deku'
          ? (ready?'연막 · READY':`연막 · ${cd.toFixed(1)}s`)
  ...
  setSkillButtonVisible(ui.skill2Btn,showSecond);
  if(showSecond){
  ...
  const showThird=cannon==='error'||cannon==='deku';
  setSkillButtonVisible(ui.skill3Btn,showThird);
  if(showThird){
    const cd3=Math.max(0,player.skill3Cd||0),ready3=cd3<=.001,def3=cannon==='deku'?DEKU_T_SKILL:ERROR_T_SKILL;
    if(ui.skill3Name)ui.skill3Name.textContent=cannon==='deku'?'발경':(player.errorSwordMode?'검 모드 ON':'GLITCH BLADE');
    if(ui.skill3Cooldown)ui.skill3Cooldown.textContent=cannon==='deku'
      ? (ready3?'발경 · READY':`발경 · ${cd3.toFixed(1)}s`)
      : (ready3?(player.errorSwordMode?'T · 검 모드 해제':'T · 검 모드 전환'):`${cd3.toFixed(1)}s`);
    if(ui.skill3Fill)ui.skill3Fill.style.width=`${clamp((1-cd3/def3.cooldown)*100,0,100)}%`;
    if(ui.skill3Btn){
      ui.skill3Btn.disabled=!ready3;
      ui.skill3Btn.className=`skill-button skill3-button cannon-${cannon} ${cannon==='error'&&player.errorSwordMode?'sword-on ':''}${ready3?'ready':'cooling'}`;
      setSkillButtonVisible(ui.skill3Btn,true);
    }
  }
  ...
  if(showFourth){
    ...
    if(ui.skill4Cooldown)ui.skill4Cooldown.textContent=ready4?'변속 · READY':`변속 · ${cd4.toFixed(1)}s`;
    if(ui.skill4Fill)ui.skill4Fill.style.width=`${clamp((1-cd4/DEKU_Y_SKILL.cooldown)*100,0,100)}%`;
    if(ui.skill4Btn){
      ui.skill4Btn.disabled=!ready4;
      ui.skill4Btn.className=`skill-button skill4-button cannon-deku ${ready4?'ready':'cooling'}`;
      setSkillButtonVisible(ui.skill4Btn,true);
    }
  }else{
  ...
  // Final guard after className assignments: class replacement must never re-hide a Deku slot.
  if(cannon==='deku'){
    for(const btn of [ui.skillBtn,ui.skill2Btn,ui.skill3Btn,ui.skill4Btn])setSkillButtonVisible(btn,true);
  }
}
...
function errorSwordArcDamage(x,y,angle,damage,range=135,halfAngle=.82){
  ...
  for(let i=shapes.length-1;i>=0;i--){
    ...
    if(s.hp<=0){gainPolygonXp(s.xp);burst(s.x,s.y,colorForShape(s.type),11);shapes.splice(i,1)}
  }
  for(const enemy of remotePlayers.values()){
    if(!enemy.alive)continue;
    const dx=enemy.x-x,dy=enemy.y-y,d2=dx*dx+dy*dy;
...
function beginHeldSkillAim(slot,source='keyboard',event=null){
  ...
  if(slot===1&&player?.cannonType==='gojo')startGojoAoHold();
...
function cancelHeldSkillAim(){
  if(player?.gojoAoHolding)finishGojoAoHold(false);
...
function releaseHeldSkillAim(slot=heldSkillAim.slot){
  ...
  const gojoTarget=cannon==='gojo'?gojoAimPoint(slot===1?480:540):null;
  if(player&&Number.isFinite(heldSkillAim.angle))player.angle=heldSkillAim.angle;
  heldSkillAim.active=false;heldSkillAim.slot=0;heldSkillAim.source='';heldSkillAim.pointerId=null;heldSkillAim.angle=null;heldSkillAim.distance=null;heldSkillAim.moved=false;
  if(cannon==='gojo'&&slot===1)return finishGojoAoHold(true);
  if(cannon==='gojo'&&slot===2)return castGojoSkill('aka',gojoTarget);
...
function gojoAimPoint(distance){
  if(heldSkillAim.source==='button'&&heldSkillAim.moved)return skillAimPoint(distance);
  const wx=camera.x+input.mouseX,wy=camera.y+input.mouseY;
  const dx=wx-player.x,dy=wy-player.y,d=Math.hypot(dx,dy);
  if(d<8)return skillAimPoint(distance);
...
function spawnSkillProjectileAt(cannon,x,y,angle,opts={}){
  ...
  const b={
    ...
    targetId:String(opts.targetId||''),targetShapeId:String(opts.targetShapeId||''),targetShapeRef:opts.targetShapeRef||null,
    hitTargets:new Set(),hitIds:new Set(),tetherHits:new Map()
  };
...
function removeLocalGojoAoZone(){
  for(let i=skillZones.length-1;i>=0;i--){const z=skillZones[i];if(z.type==='gojoAo'&&!z.networkRemote&&z.ownerId===onlineSelfId)skillZones.splice(i,1)}
}
function startGojoAoHold(){
  if(!player||player.gojoAoHolding)return false;
  const [x,y]=gojoAimPoint(480);
  player.gojoAoHolding=true;player.gojoAoNetAt=0;
  addSkillZone('gojoAo',{x,y,radius:210,life:999,damage:playerParams().damage*.42,data:{held:true}});
  sendUniqueSkill('gojo','gojoAo',{targetX:x,targetY:y,radius:210,life:.22,held:true});
  return true;
}
function finishGojoAoHold(commitCooldown=true){
  if(!player?.gojoAoHolding)return false;
  player.gojoAoHolding=false;removeLocalGojoAoZone();
  if(commitCooldown){const cd=CANNON_SKILLS.gojo.cooldown,wall=Date.now();player.skillCd=player.skillMax=cd;player.skillReadyAt=wall+cd*1000;updateSkillHud()}
  sendUniqueSkill('gojo','gojoAoEnd',{targetX:player.x,targetY:player.y,radius:20,life:.2});
  return true;
}

function castGojoSkill(kind,target=null){
...
function tryGojoPurple(){
  ...
  if(player.gojoAoHolding)finishGojoAoHold(false);cancelHeldSkillAim();return castGojoSkill('purple');
}
...
function activateSkill(errorChargeRatio=0,aimOverride=null){
  ...
  if(skillCannon==='bloodlust'){
    ...
    broadcastLocalState(true);
  }else if(skillCannon==='blaster'){
    const[tx,ty]=skillAimPoint(760),aa=Math.atan2(ty-player.y,tx-player.x);spawnSkillProjectileAt(cannon,player.x+Math.cos(aa)*42,player.y+Math.sin(aa)*42,aa,{damageMul:5.4,speedMul:1.18,life:5,pierce:1,r:15,splashRadius:185,shape:'standardSlug',special:'coreCannon'});spawnCombatFx('uniqueSkill',player.x,player.y,{angle:aa,color:def.color,life:.55,radius:110,cannon});shake=Math.max(shake,12);sendUniqueSkill(cannon,'coreCannon',{targetX:tx,targetY:ty,radius:185});
  }else if(skillCannon==='ranger'){
    const ox=player.x,oy=player.y,[tx,ty]=skillAimPoint(720),aa=Math.atan2(ty-oy,tx-ox),len=Math.hypot(tx-ox,ty-oy);for(let k=0;k<3;k++){const d=len*(k+1)/3,x=ox+Math.cos(aa)*d,y=oy+Math.sin(aa)*d;skillAreaDamage(x,y,72,p.damage*1.25,'#7ee8ff');spawnCombatFx('uniqueSkill',x,y,{angle:aa,color:def.color,life:.34,radius:72,cannon})}player.x=tx;player.y=ty;player.phaseUntil=now+520;player.vx=Math.cos(aa)*330;player.vy=Math.sin(aa)*330;broadcastLocalState(true);sendUniqueSkill(cannon,'hunterStep',{targetX:tx,targetY:ty,length:len});
  }else if(skillCannon==='ricochet'){
    const[tx,ty]=skillAimPoint(500),base=Math.atan2(ty-player.y,tx-player.x);for(let k=-2;k<=2;k++){const b=spawnSkillProjectileAt(cannon,player.x,player.y,base+k*.13,{damageMul:1.35,speedMul:1.12,life:5,pierce:4,r:8,shape:'burstDisc',special:'pinballCore'});b.curve=k*.22}spawnCombatFx('uniqueSkill',player.x,player.y,{angle:base,color:def.color,life:.7,radius:130,cannon});sendUniqueSkill(cannon,'pinballCore',{targetX:tx,targetY:ty});
  }else if(skillCannon==='mortar'){
    const[tx,ty]=skillAimPoint(650),aa=Math.atan2(ty-player.y,tx-player.x);for(let k=0;k<5;k++){const d=(k-2)*105,x=tx+Math.cos(aa)*d,y=ty+Math.sin(aa)*d;addSkillZone('mortarBarrage',{x,y,radius:125,life:.75+k*.13,damage:p.damage*2.25,data:{detonateAt:.08}})}spawnCombatFx('uniqueSkill',tx,ty,{angle:aa,color:def.color,life:.9,radius:360,cannon});sendUniqueSkill(cannon,'mortarBarrage',{targetX:tx,targetY:ty,radius:360});
  }else if(skillCannon==='shredder'){
    for(const off of [-.20,0,.20]){const b=spawnSkillProjectileAt(cannon,player.x+Math.cos(a)*38,player.y+Math.sin(a)*38,a+off,{damageMul:3.0,speedMul:.88,life:5,pierce:14,r:16,shape:'drillBit',special:'sawStorm'});b.curve=off<0?-1:off>0?1:0}spawnCombatFx('uniqueSkill',player.x,player.y,{angle:a,color:def.color,life:.8,radius:160,cannon});sendUniqueSkill(cannon,'sawStorm',{targetX:player.x+Math.cos(a)*700,targetY:player.y+Math.sin(a)*700});
  }else if(skillCannon==='seeker'){
    const target=sniperAcquireTarget(a),b=spawnSkillProjectileAt(cannon,player.x+Math.cos(a)*40,player.y+Math.sin(a)*40,a,{damageMul:5.1,speedMul:1.18,life:5,pierce:5,r:9,shape:'piercerLance',special:'seekerRail',targetId:target?.kind==='player'?target.target.id:'',targetShapeId:target?.kind==='shape'?target.target.id:''});if(target?.kind==='shape')b.targetShapeRef=target.target;spawnCombatFx('uniqueSkill',player.x,player.y,{angle:a,color:def.color,life:.65,radius:180,cannon});sendUniqueSkill(cannon,'seekerRail',{targetX:target?.x||player.x+Math.cos(a)*900,targetY:target?.y||player.y+Math.sin(a)*900});
  }else if(skillCannon==='frost'){
    const[tx,ty]=skillAimPoint(470);addSkillZone('frostPrison',{x:tx,y:ty,radius:280,life:5.4,damage:p.damage*.42,tick:0});spawnCombatFx('uniqueSkill',tx,ty,{angle:0,color:def.color,life:1.0,radius:280,cannon});sendUniqueSkill(cannon,'frostPrison',{targetX:tx,targetY:ty,radius:280,life:5.4});
  ...
  }else if(skillCannon==='sniper'){
    player.sniperScopeUntil=now+10000;
    spawnCombatFx('sniperScopePulse',player.x,player.y,{angle:a,color:'#bdefff',life:.72,radius:145,cannon:'sniper'});
    burst(player.x,player.y,'#bdefff',16);
    sendUniqueSkill(cannon,'sniperScope',{targetX:player.x,targetY:player.y,radius:145,life:.72});
  }else if(skillCannon==='deku'){
...
function sniperAcquireTarget(angle){
  let best=null,bestScore=Infinity;
  const scoreTarget=(x,y,kind,target)=>{
    const dx=x-player.x,dy=y-player.y,dist=Math.hypot(dx,dy);
    if(dist<20||dist>2800)return;
    const diff=Math.abs(angleDifference(Math.atan2(dy,dx),angle));
    const score=diff*760+dist*.32;
    if(score<bestScore){bestScore=score;best={kind,target,x,y,dist,diff}}
  };
  for(const enemy of remotePlayers.values())if(enemy?.alive)scoreTarget(enemy.x,enemy.y,'player',enemy);
  // Solo play / no visible player: guided round may also lock on to a world shape.
  if(!best||best.diff>.72){
    for(const s of shapes)if(s?.hp>0)scoreTarget(s.x,s.y,'shape',s);
  }
  return best;
}
...
function activateSkill2(){
  ...
  if(skill2Cannon==='bloodlust'){
    ...
    const ox=player.x,oy=player.y;
    const length=800,width=faJinBoost?70:58,x2=ox+Math.cos(a)*length,y2=oy+Math.sin(a)*length;
    ...
    if(hitPlayer){
      ...
      const dashDistance=faJinBoost?620:480;
      ...
      if(actualDash>4){
        recoilDash=true;
        player.phaseUntil=now+440;
        player.x=dashX;player.y=dashY;
        player.vx=Math.cos(a)*p.move*1.24;player.vy=Math.sin(a)*p.move*1.24;
        spawnCombatFx('dekuWhipElasticDash',ox,oy,{angle:a,color:'#5cf6e8',life:.68,radius:actualDash,cannon:'deku',variant:faJinBoost?'faJin':''});
...
function chainPlasmaBasicHit(parent,x,y,primaryShape=null,primaryPlayerId=''){
  ...
  const baseCannon=cannonBaseBehavior(parent.cannon),range=baseCannon==='thunder'?330:280,range2=range*range,candidates=[];
  ...
  for(const c of candidates.slice(0,baseCannon==='thunder'?3:2)){
    const tx=c.target.x,ty=c.target.y,damage=parent.damage*(baseCannon==='thunder'?.40:.45);
    ...
    if(c.kind==='shape'){applyShapeDamage(c.target,damage);burst(tx,ty,'#89f8ff',5);if(c.target.hp<=0){const idx=shapes.indexOf(c.target);if(idx>=0){gainPolygonXp(c.target.xp);burst(tx,ty,colorForShape(c.target.type),10);shapes.splice(idx,1)}}}
...
function errorGlitchImpact(parent,x,y,primaryShape=null,primaryPlayerId=''){
  ...
  for(let i=shapes.length-1;i>=0;i--){
    ...
    if(s.hp<=0){
      if(parent.owner===player)gainPolygonXp(s.xp);
...
function configureBasicProjectilePhysics(b,cannon,special=''){
  cannon=cannonBaseBehavior(cannon);
  b.r=6;b.life=1.65;b.pierce=1;b.splashRadius=0;
  if(cannon==='standard'){b.r=6;b.life=1.65}
  else if(cannon==='scout'){b.r=5;b.life=1.55;b.vx*=1.08;b.vy*=1.08}
  else if(cannon==='bastion'){b.r=10;b.life=2.15;b.pierce=special==='bastionCore'?3:1;b.splashRadius=special==='bastionCore'?52:28}
  else if(cannon==='rapid'){b.r=3.5;b.life=1.28}
  else if(cannon==='dual'){b.r=5;b.life=1.42}
  else if(cannon==='needle'){b.r=3;b.life=1.65;b.pierce=special==='needleCore'?4:2}
  ...
  else if(cannon==='laser'){b.r=4;b.life=1.50;b.pierce=special==='laserCore'?10:5}
  else if(cannon==='drill'){b.r=special==='drillCore'?11:8;b.life=special==='drillCore'?2.25:1.95;b.pierce=special==='drillCore'?16:8}
    ...
    if(special==='nullCore'){b.r=13;b.life=2.65;b.pierce=24;b.vx*=1.10;b.vy*=1.10}
    else if(special==='nullEcho'){b.r=8;b.life=2.35;b.pierce=18;b.vx*=1.05;b.vy*=1.05}
  }
...
function spawnBasicSplitFragment(parent,spec,x,y,index=0){
  ...
  const speed=Math.max(220,Math.hypot(parent.vx,parent.vy)*(Number(spec.speedMul)||.96));
  const special=String(spec.special||'');
  const b={
    x,y,vx:Math.cos(a)*speed,vy:Math.sin(a)*speed,
    r:6,damage:Math.max(0,(Number(parent.splitBaseDamage)||Number(parent.damage)||0)*(Number(spec.damageMul)||1)),life:1.65,
    owner:parent.owner||player,ownerId:onlineSelfId,team:'player',cannon:parent.cannon,shape:projectileShapeForCannon(parent.cannon),
    pierce:1,splashRadius:0,basicAttack:true,special,fragment:true,
    curve:Number(spec.curve)||0,motionSeed:((Number(parent.motionSeed)||0)+index*2.317+1.113)%TAU,motionAge:0,
    hitTargets:new Set(),hitIds:new Set(),tetherHits:new Map()
  };
  configureBasicProjectilePhysics(b,b.cannon,special);
  if(bullets.length>=MAX_BULLETS)return null;
  bullets.push(b);broadcastShot(b);
  return b;
}
function splitBasicCarrier(parent){
  if(!parent||parent.networkRemote||parent.splitDone||!Array.isArray(parent.splitPattern)||!parent.splitPattern.length)return false;
  parent.splitDone=true;
  const x=parent.x,y=parent.y,pattern=parent.splitPattern;
  for(let k=0;k<pattern.length;k++)spawnBasicSplitFragment(parent,pattern[k],x,y,k);
  const theme=TANK_THEMES[parent.cannon]||TANK_THEMES.standard;
  burst(x,y,theme.glow||'#dffcff',Math.min(18,6+pattern.length*2));
  spawnCombatFx('splitBurst',x,y,{angle:Math.atan2(parent.vy,parent.vx),color:theme.glow||'#dffcff',life:.30,radius:30+pattern.length*5,cannon:parent.cannon});
  return true;
}
...
function fire(e){
  ...
  // V5.60: 모든 평타는 포구에서 반드시 1발만 출발한다.
  if(baseCannon==='standard'){
    shot={angle:a,side:0,curve:0,damageMul:shotNo%5===0?1.70:1,special:shotNo%5===0?'precision':''};
  }else if(baseCannon==='scout'){
    shot=shotNo%4===0
      ?carrier('scoutSplitCarrier',170,[part(-.05,.84,'scoutDouble',-1),part(.05,.84,'scoutDouble',1)])
      :{angle:a,side:0,curve:shotNo%2?-1:1,damageMul:1,special:'scoutWeave'};
  }else if(baseCannon==='bastion'){
    shot={angle:a,side:0,curve:0,damageMul:shotNo%6===0?2.20:1,special:shotNo%6===0?'bastionCore':'bastionHeavy'};
  }else if(baseCannon==='rapid'){
    const jitter=Math.sin(shotNo*2.71)*.026;
    shot=shotNo%8===0
      ?carrier('rapidSplitCarrier',150,[part(-.045,.58,'acceleratedBurst',-1),part(0,.58,'acceleratedBurst',0),part(.045,.58,'acceleratedBurst',1)])
      :{angle:a+jitter,side:0,curve:Math.sin(shotNo),damageMul:1,special:'rapidJitter'};
  }else if(baseCannon==='dual'){
    shot=shotNo%7===0
      ?carrier('dualSplitCarrier',165,[part(-.07,.45,'dualBurst',-1),part(-.025,.45,'dualBurst',-1),part(.025,.45,'dualBurst',1),part(.07,.45,'dualBurst',1)])
      :carrier('dualSplitCarrier',145,[part(-.018,.68,'dualLeft',-1),part(.018,.68,'dualRight',1)]);
  }else if(baseCannon==='needle'){
    shot={angle:a,side:0,curve:0,damageMul:shotNo%6===0?1.55:1,special:shotNo%6===0?'needleCore':'needleAccel'};
  }else if(baseCannon==='spread'){
    shot=carrier('spreadCarrier',190,[-.25,-.125,0,.125,.25].map(v=>part(v,1,'shrapnelCarrier',Math.sign(v))));
  }else if(baseCannon==='burst'){
    const arr=shotNo%5===0?[-.17,-.085,0,.085,.17]:[-.075,0,.075],mul=shotNo%5===0?.55:.72;
    shot=carrier('burstCarrier',175,arr.map((v,i)=>part(v,mul,'burstDisc',i%2?-1:1)));
  }else if(baseCannon==='crystal'){
    const arr=shotNo%4===0?[-.30,-.20,-.10,0,.10,.20,.30]:[-.15,0,.15],mul=shotNo%4===0?.42:.65;
    shot=carrier('crystalCarrier',215,arr.map((v,i)=>part(v,mul,'crystalCurve',i<(arr.length/2)?-1:1)));
  }else if(baseCannon==='piercer'){
    shot={angle:a,side:0,curve:0,damageMul:1,special:'penetrationAccel'};
  }else if(baseCannon==='laser'){
    shot=shotNo%3===0
      ?{angle:a,side:0,curve:0,damageMul:1.75,special:'laserCore'}
      :carrier('laserSplitCarrier',205,[part(-.022,.58,'laserTwin',0,1),part(.022,.58,'laserTwin',0,1)]);
  }else if(baseCannon==='drill'){
    shot={angle:a,side:0,curve:shotNo%2?-1:1,damageMul:shotNo%4===0?2.20:1.05,special:shotNo%4===0?'drillCore':'drillSpin'};
  }else if(baseCannon==='plasma'){
    shot={angle:a,side:0,curve:0,damageMul:1,special:'chainDischarge'};
  }else if(baseCannon==='thunder'){
    const arr=shotNo%4===0?[-.10,0,.10]:[-.045,.045],mul=shotNo%4===0?.72:.64;
    shot=carrier('thunderCarrier',215,arr.map((v,i)=>part(v,mul,'thunderArc',i%2?-1:1)));
  }else if(baseCannon==='inferno'){
    shot=shotNo%5===0
      ?carrier('infernoCarrier',195,[-.15,0,.15].map((v,i)=>part(v,.68,'infernoTriple',i-1)))
      :{angle:a,side:0,curve:shotNo%2?-1:1,damageMul:1,special:'incendiary'};
  }else if(baseCannon==='rocket'){
    shot={angle:a,side:0,curve:0,damageMul:1,special:'incendiary'};
  }else if(baseCannon==='titan'){
    shot=shotNo%4===0
      ?carrier('titanCarrier',235,[part(-.028,.80,'titanTwin'),part(.028,.80,'titanTwin')])
      :{angle:a,side:0,curve:0,damageMul:1,special:'titanAlternating'};
  }else if(baseCannon==='phantom'){
    shot=shotNo%5===0
      ?carrier('phantomCarrier',245,[-.07,0,.07].map((v,i)=>part(v,.65,'phantomTriple',i-1)))
      :{angle:a,side:0,curve:shotNo%2?-1:1,damageMul:1,special:'phantomWave'};
  }else if(baseCannon==='ring'){
    shot={angle:a,side:0,curve:0,damageMul:1,special:'returnRing'};
  }else if(baseCannon==='chrono'){
    const arr=shotNo%3===0?[-.07,0,.07]:[-.04,.04],mul=shotNo%3===0?.56:.72;
    shot=carrier('chronoCarrier',195,arr.map((v,i)=>part(v,mul,'chronoCurve',i<(arr.length/2)?-1:1)));
  }else if(baseCannon==='void'){
    shot={angle:a,side:0,curve:0,damageMul:shotNo%4===0?1.35:1,special:shotNo%4===0?'voidPulse':'voidGravity'};
  }else if(baseCannon==='nova'){
    shot=carrier('novaCarrier',225,[-.10,0,.10].map(v=>part(v,.78,'nebulaCarrier',Math.sign(v))));
  }else if(baseCannon==='comet'){
    shot=shotNo%5===0
      ?carrier('cometCarrier',235,[-.10,0,.10].map((v,i)=>part(v,.70,'cometTriple',i-1)))
      :{angle:a,side:0,curve:0,damageMul:1,special:'cometAccel'};
  }else if(baseCannon==='stellar'){
    const arr=shotNo%6===0?[-.16,-.08,0,.08,.16]:[-.055,.055],mul=shotNo%6===0?.42:.66;
    shot=carrier('stellarCarrier',220,arr.map((v,i)=>part(v,mul,'stellarWave',i<(arr.length/2)?-1:1)));
  }else if(baseCannon==='error'){
    shot=shotNo%5===0
      ?carrier('errorCarrier',230,[
        part(-.18,.55,'nullEcho',-1),part(-.09,.72,'nullEcho',-1),part(0,1.65,'nullCore',0),part(.09,.72,'nullEcho',1),part(.18,.55,'nullEcho',1)
      ])
      :{angle:a,side:0,curve:shotNo%2?-1:1,damageMul:1,special:'errorGlitch'};
  }else if(baseCannon==='glitch'){
    const arr=shotNo%6===0?[-.18,-.12,-.06,0,.06,.12,.18]:[-.045,.045],mul=shotNo%6===0?.35:.62;
    shot=carrier('glitchCarrier',185,arr.map((v,i)=>part(v,mul,'glitchPacket',i%2?-1:1)));
  }else if(baseCannon==='zero'){
    shot=shotNo%4===0
    ...
    const faJinBoost=performance.now()<(e.dekuFaJinUntil||0),smash=shotNo%5===0;
    shot={angle:a,side:0,curve:0,damageMul:smash?2.40:1,special:faJinBoost?(smash?'faJinDetroitAirSmash':'faJinAirForce'):(smash?'detroitAirSmash':'airForce')};
  }else{
...
function applySplashDamage(b,x,y){
  ...
  if(b.basicAttack&&['rocket','inferno'].includes(cannonBaseBehavior(b.cannon)))createRocketBurnZone(b,x,y);
  const baseCannon=cannonBaseBehavior(b.cannon),radius=b.splashRadius,r2=radius*radius,base=b.damage*(baseCannon==='rocket'?.72:.58);
  for(let i=shapes.length-1;i>=0;i--){
    ...
    if(s.hp<=0){
      if(b.owner===player)gainPolygonXp(s.xp);
      burst(s.x,s.y,colorForShape(s.type),10);
      shapes.splice(i,1);
    }
  }
  ...
  burst(x,y,baseCannon==='rocket'?'#ffb55f':'#69e6ff',baseCannon==='rocket'?22:14);
  shake=Math.max(shake,baseCannon==='rocket'?8:4);
}
...
function applyUniqueProjectileMotion(b,dt){
  ...
  if(/Carrier$/.test(String(b.special||'')))return;
  if(String(b.special||'')==='seekerRail'){
  ...
  for(let i=bullets.length-1;i>=0;i--){
    ...
    for(let j=shapes.length-1;j>=0&&!remove;j--){
      ...
      if((b.x-s.x)**2+(b.y-s.y)**2<rr*rr){
        ...
        if(b.basicAttack&&['plasma','thunder'].includes(cannonBaseBehavior(b.cannon)))chainPlasmaBasicHit(b,b.x,b.y,s,'');
        ...
        if(b.splashRadius){applySplashDamage(b,b.x,b.y);if(cannonBaseBehavior(b.cannon)!=='sniper'){remove=true;continue}}
        if((b.pierce||1)>1){
          if(cannonBaseBehavior(b.cannon)!=='sniper')b.pierce--;
          if(b.basicAttack&&cannonBaseBehavior(b.cannon)==='piercer'){
    ...
    if(!remove&&b.team==='player'){
      for(const enemy of remotePlayers.values()){
        ...
        if((b.x-enemy.x)**2+(b.y-enemy.y)**2<rr*rr){
          ...
          if(b.basicAttack&&cannonBaseBehavior(b.cannon)==='spread')spawnSpreadFragments(b,b.x,b.y);
          if(b.basicAttack&&cannonBaseBehavior(b.cannon)==='nova')spawnNovaFragments(b,b.x,b.y);
          if(b.basicAttack&&['plasma','thunder'].includes(cannonBaseBehavior(b.cannon)))chainPlasmaBasicHit(b,b.x,b.y,null,enemy.id);
          if(b.basicAttack&&cannonFamily(b.cannon)==='error')errorGlitchImpact(b,b.x,b.y,null,enemy.id);
          if(b.splashRadius){applySplashDamage(b,b.x,b.y);if(cannonBaseBehavior(b.cannon)!=='sniper'){remove=true;break}}
          if((b.pierce||1)>1){
            if(cannonBaseBehavior(b.cannon)!=='sniper')b.pierce--;
            if(b.basicAttack&&cannonBaseBehavior(b.cannon)==='piercer'){
            b.damage*=1.08;b.vx*=1.04;b.vy*=1.04;
            broadcastShotSync(b);
            spawnCombatFx('pierceAccel',b.x,b.y,{angle:Math.atan2(b.vy,b.vx),color:'#c8a8ff',life:.24,radius:28,cannon:'piercer'});
          }else if(b.basicAttack&&cannonBaseBehavior(b.cannon)==='error'){
            b.damage*=1.04;b.vx*=1.02;b.vy*=1.02;
            broadcastShotSync(b);
            spawnCombatFx('errorPierce',b.x,b.y,{angle:Math.atan2(b.vy,b.vx),color:'#72ff43',life:.20,radius:30,cannon:'error'});
          }
...
function updateSkillZones(dt){
  ...
  for(let zi=skillZones.length-1;zi>=0;zi--){
    ...
    if(z.type==='gojoAo'){
      if(z.data?.held&&player?.gojoAoHolding){
        const [tx,ty]=gojoAimPoint(480);z.x=tx;z.y=ty;z.life=z.maxLife=999;
        player.gojoAoNetAt=(player.gojoAoNetAt||0)-dt;
        if(player.gojoAoNetAt<=0){player.gojoAoNetAt=.10;sendUniqueSkill('gojo','gojoAo',{targetX:z.x,targetY:z.y,radius:z.radius,life:.22,held:true})}
      }
      z.tick=(z.tick||0)+dt;
      if(z.tick>=.055){
        z.tick=0;
        for(const s of shapes){const dx=z.x-s.x,dy=z.y-s.y,d=Math.hypot(dx,dy)||1;if(d<z.radius*1.15)reportShapeImpulse(s,dx/d*85,dy/d*85)}
      }
      z.data.damageTick=(z.data.damageTick||0)+dt;
      if(z.data.held&&z.data.damageTick>=.14){z.data.damageTick=0;skillAreaDamage(z.x,z.y,z.radius,z.damage,'#62b8ff')}
      continue;
    }
    if(z.type==='gojoAkaImpact'){
      if(!z.triggered&&elapsed>=(z.data.delay||.24)){
        z.triggered=true;skillAreaDamage(z.x,z.y,z.radius,z.damage,'#ff7083');
    ...
    if(z.type==='gojoPurpleProjectile'){
      const charge=z.data.charge||.72,flight=z.data.flight||.78;
      if(elapsed<charge)continue;
      const progress=clamp((elapsed-charge)/flight,0,1),prev=z.data.prevProgress??progress;
      const x1=z.x+Math.cos(z.angle)*z.length*prev,y1=z.y+Math.sin(z.angle)*z.length*prev;
      const x2=z.x+Math.cos(z.angle)*z.length*progress,y2=z.y+Math.sin(z.angle)*z.length*progress;
      z.data.prevProgress=progress;
      for(let i=shapes.length-1;i>=0;i--){const s=shapes[i];if(z.data.hitShapes.has(s))continue;const hit=pointToLineInfo(s.x,s.y,x1,y1,x2,y2);if(hit.d2>(z.radius+s.r)**2)continue;z.data.hitShapes.add(s);applyShapeDamage(s,z.damage,Math.cos(z.angle)*140,Math.sin(z.angle)*140);burst(s.x,s.y,'#c38aff',12);if(s.hp<=0){gainPolygonXp(s.xp);burst(s.x,s.y,colorForShape(s.type),14);shapes.splice(i,1)}}
      for(const enemy of remotePlayers.values()){if(!enemy.alive||z.data.hitPlayers.has(enemy.id))continue;const hit=pointToLineInfo(enemy.x,enemy.y,x1,y1,x2,y2);if(hit.d2>(z.radius+(enemy.r||27))**2)continue;z.data.hitPlayers.add(enemy.id);sendDamage(enemy.id,z.damage)}
      continue;
    }
    ...
    if(z.type==='constellation'){
      ...
      if(z.tick<=0){
        ...
        for(let i=shapes.length-1;i>=0;i--){const s=shapes[i];if(!pointInTriangle(s.x,s.y,ax,ay,bx,by,cx,cy))continue;applyShapeDamage(s,z.damage);burst(s.x,s.y,'#e8ffff',3);if(s.hp<=0){gainPolygonXp(s.xp);shapes.splice(i,1)}}
    ...
    if(z.type==='mortarBarrage'||z.type==='fateMark'||z.type==='threeFates'){
      if(!z.triggered&&z.life<(z.data.detonateAt||.10)){z.triggered=true;skillAreaDamage(z.x,z.y,z.radius,z.damage,z.type==='mortarBarrage'?'#ffc27d':'#fff2ae');burst(z.x,z.y,z.type==='mortarBarrage'?'#ffc27d':'#fff2ae',26);shake=Math.max(shake,9)}continue;
    }
    if(z.type==='frostPrison'){
      z.tick=(z.tick||0)-dt;if(z.tick<=0){z.tick=.18;for(const s of shapes){const dx=s.x-z.x,dy=s.y-z.y;if(dx*dx+dy*dy<=z.radius*z.radius){s.vx*=.18;s.vy*=.18;applyShapeDamage(s,z.damage)}}for(const b of bullets){if(b.team==='remote'&&(b.x-z.x)**2+(b.y-z.y)**2<=z.radius*z.radius){b.vx*=.40;b.vy*=.40}}}continue;
    }
    ...
    if(z.type==='einherjarRain'){z.data.next=(z.data.next||0)-dt;if(z.data.next<=0&&(z.data.count||0)<18){z.data.next=.28;z.data.count=(z.data.count||0)+1;const q=Math.random()*TAU,rr=Math.sqrt(Math.random())*z.radius,x=z.x+Math.cos(q)*rr,y=z.y+Math.sin(q)*rr;skillAreaDamage(x,y,70,z.damage,'#b9ecff');spawnCombatFx('uniqueSkill',x,y,{angle:Math.PI/2,color:'#b9ecff',life:.34,radius:76,cannon:'valkyrie'})}continue;}
    if(z.type==='lastBlood'){z.x=player.x;z.y=player.y;z.data.next=(z.data.next||0)-dt;if(z.data.next<=0){z.data.next=.24;const missing=1-player.hp/Math.max(1,player.maxHp);skillAreaDamage(z.x,z.y,z.radius,z.damage*(1+missing*1.2),'#ff8a70')}continue;}
...
function drawGrid(){const b=viewScreenBounds(80);ctx.fillStyle='#152235';ctx.fillRect(b.left,b.top,b.right-b.left,b.bottom-b.top);let sx=-(camera.x%GRID),sy=-(camera.y%GRID);while(sx>b.left)sx-=GRID;while(sy>b.top)sy-=GRID;ctx.strokeStyle='rgba(255,255,255,.045)';ctx.lineWidth=1;ctx.beginPath();for(let x=sx;x<b.right;x+=GRID){ctx.moveTo(x,b.top);ctx.lineTo(x,b.bottom)}for(let y=sy;y<b.bottom;y+=GRID){ctx.moveTo(b.left,y);ctx.lineTo(b.right,y)}ctx.stroke()}
...
function screenVisibleWorld(x,y,pad=100){
  const sx=x-camera.x,sy=y-camera.y,b=viewScreenBounds(pad);
  return sx>=b.left&&sy>=b.top&&sx<=b.right&&sy<=b.bottom;
}
function segmentMayTouchScreen(x1,y1,x2,y2,pad=120){
  const ax=x1-camera.x,ay=y1-camera.y,bx=x2-camera.x,by=y2-camera.y,b=viewScreenBounds(pad);
  if(ax<b.left&&bx<b.left)return false;
  if(ay<b.top&&by<b.top)return false;
  if(ax>b.right&&bx>b.right)return false;
...
function drawPlayerCannon(cannon,r){
  ...
  switch(cannonBaseBehavior(cannon)){
    ...
    case 'sniper':
      ...
      ctx.fillStyle='#9feaff';ctx.beginPath();ctx.arc(r+25,-12,5,0,TAU);ctx.fill();break;
    case 'error':
...
function drawVariantArmor(cannon,r,t){
  ...
  const v=variants[cannon]||variants[cannonBaseBehavior(cannon)];if(!v)return;
  const[kind,count]=v;
  const theme=TANK_THEMES[cannon]||TANK_THEMES.standard;
...
function drawUniqueTankBody(cannon,r,t,theme){
  ...
  switch(cannon){
    ...
    case 'sniper':
      ...
      ctx.fillStyle='#e6fbff';ctx.beginPath();ctx.arc(0,0,4,0,TAU);ctx.fill();break;
...
function drawEvolutionChassis(e,r){
  ...
  if(smasher){
    ...
    ctx.fillStyle='rgba(203,211,220,.90)';ctx.strokeStyle='rgba(82,96,110,.95)';ctx.lineWidth=2;
...
function drawTank(e){
  ...
  const[x,y]=worldToScreen(e.x,e.y),vb=viewScreenBounds(165);
  if(x<vb.left||y<vb.top||x>vb.right||y>vb.bottom)return;
...
function drawPlasmaTethers(){
  ...
  for(const b of bullets){
    ...
    const vb=viewScreenBounds(180);if((ax<vb.left&&bx<vb.left)||(ay<vb.top&&by<vb.top)||(ax>vb.right&&bx>vb.right)||(ay>vb.bottom&&by>vb.bottom))continue;
...
function skillRuneColor(z){
  ...
  if(/error|deku|needle|rapid|mine/i.test(name))return '#baff8e';
  return '#80eaff';
}
...
function drawSkillZones(){
  ...
  for(const z of skillZones){
    ...
    if(z.type==='gojoAkaProjectile'){
      ...
      if(z.type==='gojoAkaImpact'&&(z.maxLife-z.life)<(z.data.delay||.24)){ctx.restore();continue}
      ctx.shadowColor=color;ctx.shadowBlur=blue?30:38;
      if(blue){
        const core=18+Math.sin(t*15)*4;
        ...
        flare.addColorStop(0,'rgba(255,255,255,1)');flare.addColorStop(.14,'rgba(255,196,203,.98)');flare.addColorStop(.38,'rgba(255,58,88,.72)');flare.addColorStop(1,'rgba(120,0,24,0)');
        ctx.fillStyle=flare;ctx.beginPath();ctx.arc(0,0,Math.max(core,blast),0,TAU);ctx.fill();
        ctx.strokeStyle='rgba(255,183,192,.78)';ctx.lineWidth=3*p;
        for(let k=0;k<16;k++){const a=k*TAU/16+t*.18,j=z.radius*(.58+.18*Math.sin(k*4.7));ctx.beginPath();ctx.moveTo(Math.cos(a)*blast*.7,Math.sin(a)*blast*.7);ctx.lineTo(Math.cos(a)*j,Math.sin(a)*j);ctx.stroke()}
      }
      ctx.shadowBlur=0;
...
function drawCombatEffects(layer='base'){
  ...
  for(const f of combatFx){
    ...
    if(f.type==='gojoPurple'){
      const length=f.radius,travel=length*(1-Math.pow(p,3)),head=clamp(travel,0,length),tail=Math.max(0,head-330);
      ctx.lineCap='round';ctx.shadowColor='#9c5cff';ctx.shadowBlur=36;
      for(let k=0;k<11;k++){
        const phase=k*.83+t*2.2,spread=(k-5)*9*(.35+p),wave=Math.sin(phase)*22*p;
        ctx.strokeStyle=k%3===0?'rgba(255,96,148,.30)':k%3===1?'rgba(82,162,255,.30)':'rgba(184,101,255,.36)';ctx.lineWidth=5+(k%4)*3;
        ctx.beginPath();ctx.moveTo(tail,spread*.35);ctx.bezierCurveTo(tail+(head-tail)*.35,wave+spread,tail+(head-tail)*.72,-wave+spread*.5,head-12,spread*.12);ctx.stroke();
      }
      const wake=ctx.createLinearGradient(tail,0,head,0);wake.addColorStop(0,'rgba(42,16,86,0)');wake.addColorStop(.45,'rgba(109,54,183,.25)');wake.addColorStop(1,'rgba(227,182,255,.92)');
      ctx.strokeStyle=wake;ctx.lineWidth=48*p+18;ctx.beginPath();ctx.moveTo(tail,0);ctx.lineTo(head,0);ctx.stroke();
      const orb=34+7*Math.sin(t*31),g=ctx.createRadialGradient(head-8,-8,2,head,0,orb);
      g.addColorStop(0,'#ffffff');g.addColorStop(.18,'#f4d9ff');g.addColorStop(.48,'#a760e8');g.addColorStop(.78,'#4a176f');g.addColorStop(1,'rgba(28,3,55,0)');
      ctx.fillStyle=g;ctx.beginPath();ctx.arc(head,0,orb,0,TAU);ctx.fill();
      ctx.strokeStyle='rgba(248,223,255,.8)';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(head-orb*1.2,-orb*.35);ctx.quadraticCurveTo(head,orb*.2,head+orb*.9,-orb*.15);ctx.stroke();
      ctx.shadowBlur=0;
    }else if(f.type==='gojoAkaFlight'){
      const travel=f.radius*q;ctx.shadowColor='#ff547a';ctx.shadowBlur=32;
      const tail=ctx.createLinearGradient(Math.max(0,travel-130),0,travel,0);
      tail.addColorStop(0,'rgba(255,68,104,0)');tail.addColorStop(1,'rgba(255,136,152,.85)');
      ctx.strokeStyle=tail;ctx.lineWidth=20*p+6;ctx.beginPath();ctx.moveTo(Math.max(0,travel-130),0);ctx.lineTo(travel,0);ctx.stroke();
      const orb=22+5*Math.sin(t*42),g=ctx.createRadialGradient(travel-5,-5,2,travel,0,orb);
      g.addColorStop(0,'#fff');g.addColorStop(.2,'#ffc6ce');g.addColorStop(.55,'#ff375e');g.addColorStop(1,'rgba(126,0,29,0)');
      ctx.fillStyle=g;ctx.beginPath();ctx.arc(travel,0,orb,0,TAU);ctx.fill();
      for(let k=0;k<7;k++){const a=t*6+k*TAU/7,rr=orb*(1.35+.18*Math.sin(k+t*9));ctx.fillStyle='rgba(255,126,146,.65)';ctx.beginPath();ctx.arc(travel+Math.cos(a)*rr,Math.sin(a)*rr,2.5,0,TAU);ctx.fill()}ctx.shadowBlur=0;
...
function drawUniqueProjectile(b,time,phase){
  ...
  if(shape==='sniperRound'){
    ...
    ctx.strokeStyle='#4ebde6';ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(-76,-2);ctx.lineTo(7,-2);ctx.moveTo(-76,2);ctx.lineTo(7,2);ctx.stroke();
    ctx.fillStyle='#ffffff';ctx.beginPath();ctx.moveTo(23,0);ctx.lineTo(7,-4);ctx.lineTo(7,4);ctx.closePath();ctx.fill();ctx.shadowBlur=0;return true;
  }
  if(shape==='sniperHomingMissile'){
    const pulse2=.7+.3*Math.sin(time*12);ctx.shadowColor='#78dcff';ctx.shadowBlur=18;
    ctx.fillStyle='#dff8ff';ctx.strokeStyle='#4d9ebd';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(20,0);ctx.lineTo(5,-8);ctx.lineTo(-13,-6);ctx.lineTo(-17,0);ctx.lineTo(-13,6);ctx.lineTo(5,8);ctx.closePath();ctx.fill();ctx.stroke();
    ctx.fillStyle='#71d7ff';ctx.beginPath();ctx.moveTo(-13,-5);ctx.lineTo(-31,-10*pulse2);ctx.lineTo(-17,0);ctx.lineTo(-31,10*pulse2);ctx.lineTo(-13,5);ctx.closePath();ctx.fill();
    ctx.strokeStyle='rgba(220,250,255,.9)';ctx.lineWidth=1.5;ctx.beginPath();ctx.arc(3,0,4,0,TAU);ctx.stroke();ctx.shadowBlur=0;return true;
  }
  if(shape==='laserRay'){
...
function drawBullets(){
  ...
  for(const b of bullets){
    ...
    if(renderPressure<2){
      ...
      ctx.save();ctx.globalAlpha=b.networkRemote?.34:.52;ctx.strokeStyle=glow;
      ctx.lineWidth=Math.max(1.5,b.r*.34);ctx.lineCap='round';
      ctx.beginPath();ctx.moveTo(-tail,0);ctx.lineTo(-b.r*.25,0);ctx.stroke();
      ctx.globalAlpha*=.65;ctx.lineWidth=1;
      ctx.beginPath();ctx.moveTo(-tail*.72,-b.r*.7);ctx.lineTo(-b.r*.2,-b.r*.45);
      ctx.moveTo(-tail*.72,b.r*.7);ctx.lineTo(-b.r*.2,b.r*.45);ctx.stroke();ctx.restore();
    }
...
function basicAimSpec(cannon){
  ...
  return cone[cannon]?{type:'cone',range:clamp(p.bulletSpeed*life,380,1750),half:cone[cannon]}:{type:'lane',range:clamp(p.bulletSpeed*life,380,1750),width:{bastion:34,dual:32,needle:15,piercer:20,laser:16,drill:29,plasma:34,inferno:38,rocket:38,titan:46,ring:34,void:38,error:38,zero:42,deku:24,sniper:12}[cannon]||24};
}
...
function drawSkillAimGuide(){
  ...
  if(cannon==='gojo'){
    const t=performance.now()*.001,red=heldSkillAim.slot===2;
    ctx.save();ctx.shadowColor=red?'#ff547b':'#5bb9ff';ctx.shadowBlur=25;
    if(red){
      const ox=px+Math.cos(a)*(player.r+41),oy=py+Math.sin(a)*(player.r+41);
      const orb=22+Math.sin(t*13)*3,g=ctx.createRadialGradient(ox-5,oy-6,2,ox,oy,orb*1.7);
      g.addColorStop(0,'#fff');g.addColorStop(.20,'#ffd5dc');g.addColorStop(.52,'#ff4165');g.addColorStop(1,'rgba(130,0,30,0)');
      ctx.fillStyle=g;ctx.beginPath();ctx.arc(ox,oy,orb*1.7,0,TAU);ctx.fill();
      for(let k=0;k<10;k++){const aa=t*(2.5+(k%3)*.3)+k*2.399,rr=31+(k%4)*8;ctx.fillStyle='rgba(255,124,145,.65)';ctx.beginPath();ctx.arc(ox+Math.cos(aa)*rr,oy+Math.sin(aa)*rr*.7,2+(k%3),0,TAU);ctx.fill()}
    }else{
...
function drawSniperScopeOverlay(){
  if(!running||!player?.alive||player.cannonType!=='sniper')return;
  const left=Math.max(0,((player.sniperScopeUntil||0)-performance.now())/1000);
  if(left<=0)return;
  const cx=innerWidth/2,cy=innerHeight/2,rr=Math.min(innerWidth,innerHeight)*.43;
  ctx.save();ctx.strokeStyle='rgba(190,243,255,.82)';ctx.lineWidth=2;ctx.setLineDash([10,9]);ctx.beginPath();ctx.arc(cx,cy,rr,0,TAU);ctx.stroke();ctx.setLineDash([]);
  ctx.strokeStyle='rgba(215,250,255,.72)';ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(cx-rr,cy);ctx.lineTo(cx+rr,cy);ctx.moveTo(cx,cy-rr);ctx.lineTo(cx,cy+rr);ctx.stroke();
  ctx.fillStyle='rgba(220,250,255,.92)';ctx.font='700 13px system-ui';ctx.textAlign='center';ctx.fillText(`3× SCOPE  ${left.toFixed(1)}s`,cx,cy-rr-12);ctx.restore();
}
function render(){
  updateRenderPressure();
  ctx.save();
  if(shake>0){ctx.translate(rand(-shake,shake),rand(-shake,shake));shake*=.86}
  const viewScale=sniperViewScale();
  if(viewScale!==1){ctx.translate(innerWidth/2,innerHeight/2);ctx.scale(viewScale,viewScale);ctx.translate(-innerWidth/2,-innerHeight/2)}
